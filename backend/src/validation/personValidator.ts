...
// Reguły walidacji dla dodawania i aktualizacji osoby
export const addPersonValidation = [
  check('gender')
    ...
    .notEmpty()
    .withMessage('Płeć jest wymagana'),
  check('firstName').isString().withMessage('Imię musi być tekstem').notEmpty().withMessage('Imię jest wymagane'),
  check('middleName').optional().isString().withMessage('Drugie imię musi być tekstem'),
  check('lastName').isString().withMessage('Nazwisko musi być tekstem').notEmpty().withMessage('Nazwisko jest wymagane'),
