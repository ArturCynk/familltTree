...
const [gender, setGender] = useState<'male' | 'female' | 'non-binary'>(initialGender || 'male');
  ...
  if (lastName.trim()) {
    // Sprawdź czy nazwisko zawiera tylko litery, spacje, myślniki i apostrofy
    const nameRegex = /^[a-zA-ZąćęłńóśźżĄĆĘŁŃÓŚŹŻ\s\-']+$/;
    if (!nameRegex.test(lastName)) {
      detected.push(`Nazwisko zawiera niedozwolone znaki.`);
    }
    
    // Sprawdź długość nazwiska
    if (lastName.trim().length < 2) {
      detected.push(`Nazwisko wydaje się zbyt krótkie.`);
    }
    
    // Sprawdź czy nazwisko nie jest pisane WIELKIMI LITERAMI
    ...
    <>
      ...
      {showInconsistencyModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl mx-4 overflow-hidden">
