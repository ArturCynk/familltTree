...
const PersonModal: React.FC<PersonModalProps> = ({ id, onClose,idTree }) => {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.type === 'init') {
        ws.send(JSON.stringify({ type: 'getPerson', payload: { id } }));
        return;
      }
      ...
      switch (msg.type) {
        ...
        case 'personUpdated':
          setPerson(msg.data);
          setFormData(msg.data);
          toast.info('Dane osoby zostały zaktualizowane na serwerze.');
          onClose(); // opcjonalne – jeśli chcesz zamknąć po edycji
          break;

        case 'personDeleted':
          toast.success('Osoba została usunięta.');
          onClose();
          break;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData || !wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) return;
  ...
  const handleDelete = () => {
    if (isDeleting || !wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) return;
